...
# try_get_by_name 新增特性 过滤器 --> 获取同级关系 要获取的值太多需要通过同级关系判断的情况
import json
...
data = json.loads(s)
print(try_get_by_name(data, 'name', filter=["status_code=0"]))  # ['微头条创作收益', '问答创作收益', '问答原创', '图文赞赏', '视频赞赏', '热点图库', '自营广告', '付费专栏', '头条抽奖', '商品卡', '付费直播', 'VIP客服', '创作者社群', '成就表彰']
print(try_get_by_name(data, 'name', filter=["status_code=3"]))  # ['文章创作收益', '文章原创', '视频创作收益', '视频原创']
print(try_get_by_name(data, 'name', filter=["status_code<3", "type=4"]))  # ['自营广告']
